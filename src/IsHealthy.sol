...
contract IsHealthy is Initializable, ContextUpgradeable, PausableUpgradeable, UUPSUpgradeable, AccessControlUpgradeable {
     ...
     * @param _router The lending pool address
     ...
     */
    function checkLiquidatable(address _user, address _router) public view returns (bool, uint256, uint256, uint256) {
        uint256 borrowValue = _userBorrowValue(_router, _borrowToken(_router), _user);
        if (borrowValue == 0) return (true, 0, 0, 0);
        (, uint256 maxCollateralValue) = _userCollateralStats(_router, _collateralToken(_router), _user);
        return (borrowValue > maxCollateralValue, borrowValue, maxCollateralValue, liquidationBonus[_router]);
    }
