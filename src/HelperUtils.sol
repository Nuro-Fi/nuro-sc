...
contract HelperUtils is Ownable {
    ...
    function getTokenValue(address _token) public view returns (uint256) {
        (, uint256 tokenPrice,,,) = ITokenDataStream(_tokenDataStream()).latestRoundData(_token);
    ...
    /**
     * @notice Gets the address position of a user in a lending pool
     * @param _lendingPool Address of the lending pool
     * @param _user Address of the user
     * @return Address of the user's position
     */
    function getAddressPosition(address _lendingPool, address _user) public view returns (address) {
     ...
     * @return Tuple containing liquidatable status, liquidation price, liquidation amount, and liquidation fee
     */
    function isLiquidatable(address user, address lendingPool) public view returns (bool, uint256, uint256, uint256) {
        address router = address(_router(lendingPool));
        return IIsHealthy(_isHealthy()).checkLiquidatable(user, router);
    }

    function getCollateralValue(address _lendingPool, address _user) public view returns (uint256) {
        return _calculateCollateralValue(_lendingPool, _user);
    }
    ...
    /**
     * @notice Internal function to get the OFT borrow token address
     * @param _lendingPool Address of the lending pool
     * @return Address of the OFT borrow token
     */
