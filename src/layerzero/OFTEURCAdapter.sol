...
/**
 * @title OFTEURCadapter
 * @notice LayerZero OFT Adapter for EURC token cross-chain transfers on Arc ecosystem
 * @dev Enables bridging of EURC tokens between chains with lock/unlock on Arc (hub chain) and mint/burn on spoke chains
 */
contract OFTEURCadapter is OFTAdapter, ReentrancyGuard, ChainSettlement {
    ...
    /**
     * @notice Constructs the OFT EURC Adapter
     * @param _token Address of the EURC token
     ...
     * @return Number of shared decimals (6 for EURC)
     */
     ...
     * @dev On Arc (SETTLEMENT_CHAIN_ID), transfers from adapter. On spoke chains, mints nEURC.
     */
     ...
     * @dev On Arc (SETTLEMENT_CHAIN_ID), locks EURC in adapter. On spoke chains, burns nEURC.
     */
    function _debit(
        address _from,
        uint256 _amountLd,
