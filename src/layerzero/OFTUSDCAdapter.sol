 ...
 * @notice LayerZero OFT Adapter for USDC token cross-chain transfers on Arc ecosystem
 * @dev Enables bridging of USDC tokens between chains with lock/unlock on Arc (hub chain)
 *      and mint/burn on destination chains (Base, Arbitrum, etc.)
 */
contract OFTUSDCadapter is OFTAdapter, ReentrancyGuard, ChainSettlement {
    ...
    /**
     * @notice Constructs the OFT USDC Adapter
     * @param _token Address of the USDC token
     ...
     * @return Number of shared decimals (6 for USDC)
     */
     ...
     * @dev On Arc (SETTLEMENT_CHAIN_ID), transfers from adapter. On spoke chains, mints nUSDC.
     */
     ...
     * @dev On Arc (SETTLEMENT_CHAIN_ID), locks USDC in adapter. On spoke chains, burns nUSDC.
     */
    function _debit(
        address _from,
        uint256 _amountLd,
