 ...
 */
contract NuroTOKEN is Initializable, ContextUpgradeable, ERC20Upgradeable, PausableUpgradeable, UUPSUpgradeable, AccessControlUpgradeable {
    // =============================================================
    //                           CONSTANTS
    // =============================================================
    ...
    /// @dev keccak256(abi.encode(uint256(keccak256("nuro.storage.NuroTOKEN")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant NUROTOKENSTORAGELOCATION = 0x5d3e2e8c88f3e0c8f5b0e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e500;

    /// @notice Error thrown when a non-operator attempts to call operator-only functions
    error NotOperator();

    /// @custom:storage-location erc7201:nuro.storage.NuroTOKEN
    struct NuroTokenStorage {
        uint8 _decimals;
    }
     ...
     */
    function _getNuroTokenStorage() private pure returns (NuroTokenStorage storage $) {
        assembly {
            $.slot := NUROTOKENSTORAGELOCATION
        }
    }

    /**
     * @notice Initializes the NuroTOKEN contract
    ...
    /**
     * @notice Initializes the NuroTOKEN token contract
    ...
    function initialize(string memory _date, uint8 _decimals) public initializer {
        __ERC20_init(string.concat("NuroTOKEN-", _date), string.concat("NuroTOKEN-", _date));
        ...
        NuroTokenStorage storage $ = _getNuroTokenStorage();
        $._decimals = _decimals;
    }
