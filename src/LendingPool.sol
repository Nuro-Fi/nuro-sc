...
import { INuroEmitter } from "./interfaces/INuroEmitter.sol";
    ...
    SwapHook
{
    ...
    function _createPosition(address _user) internal {
        address position = _addressPositions(_user);
        if (position != address(0)) revert PositionAlreadyCreated(position);
        ILPRouter(router).createPosition(_user);
        INuroEmitter(_nuroEmitter()).positionCreated(address(this), router, _user, position);
    }
    ...
    /**
     * @notice Gets the NuroEmitter address from factory
     * @return NuroEmitter address
     */
    function _nuroEmitter() internal view returns (address) {
        return IFactory(_factory()).nuroEmitter();
    }
    ...
    function _supplyLiquidityTransfer(uint256 _amount) internal {
        ...
        if (borrowToken == address(1)) {
            ...
            IERC20(borrowToken).safeTransferFrom(_msgSender(), address(this), _amount);
        }
    }
    ...
    function _withdrawLiquidityTransfer(uint256 amount) internal {
        ...
        if (borrowToken == address(1)) {
            ...
            IERC20(borrowToken).safeTransfer(_msgSender(), amount);
        }
    }
    ...
    function _supplyCollateralTransfer(address _user, uint256 _amount) internal {
        ...
        if (collateralToken == address(1)) {
            ...
            address wrappedNative = _wrappedNative();
            IWrappedNative(wrappedNative).deposit{ value: msg.value }();
            IERC20(wrappedNative).approve(position, _amount);
            IERC20(wrappedNative).safeTransfer(position, _amount);
        } else {
            IERC20(collateralToken).safeTransferFrom(_user, position, _amount);
        }
    }
    ...
    function _borrowDebtCrosschain(uint256 _userAmount, uint256 _creatorFee, uint256 _protocolFee, BorrowParams memory params) internal {
        address borrowToken = _borrowToken();
        IERC20(borrowToken).safeTransfer(_protocol(), _protocolFee);
        IERC20(borrowToken).safeTransfer(creator, _creatorFee);
        IERC20(borrowToken).approve(_oftBorrowToken(), _userAmount);
        params.sendParam.amountLD = _userAmount;
        params.sendParam.minAmountLD = 0; // TODO: fix this
    ...
    function _borrowDebtTransfer(uint256 _userAmount, uint256 _creatorFee, uint256 _protocolFee) internal {
        ...
        if (borrowToken == address(1)) {
            ...
            IERC20(borrowToken).safeTransfer(_protocol(), _protocolFee);
            IERC20(borrowToken).safeTransfer(creator, _creatorFee);
            IERC20(borrowToken).safeTransfer(_msgSender(), _userAmount);
        }
    }
    ...
    function _repayWithSelectedTokenTransfer(RepayParams memory params, uint256 borrowAmount) internal {
        ...
        SwapParams memory swapParams = SwapParams({
            tokenIn: params.token, tokenOut: borrowToken, amountIn: borrowAmount, amountOutMinimum: params.amountOutMinimum, fee: params.fee
        });

        if (params.fromPosition) {
            IPosition(_addressPositions(params.user)).repayWithSelectedToken(swapParams);
        } else {
            if (params.token != borrowToken) {
                ...
                _isNativeTransferFrom(params.user, borrowToken, borrowAmount);
            }
        }
    }
    ...
    /**
     * @notice Internal function to ensure non-zero amount
