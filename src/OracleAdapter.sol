 ...
 * @dev This contract acts as an adapter between the Nuro protocol and Orakl Network's price feeds,
...
contract OracleAdapter is Ownable {
    ...
    function latestRoundData() public view returns (uint80, uint256, uint256, uint256, uint80) {
        ...
        IStdReference.ReferenceData memory data = IStdReference(oracle).getReferenceData("USDC", "USD");
        // return (idRound, uint256(priceAnswer), startedAt, updated, answeredInRound);
        return (roundId, (data.rate / 1e10), data.lastUpdatedBase, data.lastUpdatedQuote, answeredInRound);
    }
