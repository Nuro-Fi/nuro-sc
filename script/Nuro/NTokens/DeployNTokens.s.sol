...
import { NTOKEN } from "@src/BridgeToken/NTOKEN.sol";
...
contract DeployNTokens is Script, SelectRpc, Helper {
    ...
    function _deployNToken(string memory _name, string memory _symbol, uint8 _decimals) internal {
        ntoken = new NTOKEN();
        console.log("address public constant %s_%s_IMPLEMENTATION = %s;", chainName, _symbol, address(ntoken));
        bytes memory data = abi.encodeWithSelector(ntoken.initialize.selector, _name, _symbol, _decimals);
        proxy = new ERC1967Proxy(address(ntoken), data);
        ntoken = NTOKEN(address(proxy));
    ...
    function _setRoleNTokenToElevated(address _ntoken, address _elevated) internal virtual {
        NTOKEN(_ntoken).grantRole(MINTER_ROLE, _elevated);
    }
...
// RUN
// forge script DeployNTokens --broadcast -vvv --verify --verifier etherscan --etherscan-api-key $ETHERSCAN_API_KEY
// forge script DeployNTokens --broadcast -vvv --verify
// forge script DeployNTokens --broadcast -vvv
// forge script DeployNTokens -vvv
